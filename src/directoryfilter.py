...
#filters command line argument entries or cache files, checks if they exist and accessible and if they are not the same directory as the source
def dirs_filter(directories, message="added"):
    filtered_directories = list(set([dirs for dirs in directories 
                                    if is_dir_exist_and_accessible(dirs)
...
#filters command line argument entries, adds newly specified folders exclusively

...
def dirs_filter_message(directories, filtered_directories, message):
    ...
    if args.verbose:
        ...
        for num, dirs in enumerate(lambdas.list_item_common_remove(
                set(directories), set(filtered_directories)), 1):
...
def dirs_new_filter_message(existing_directories, filtered_new_directories):
    ...
    if args.verbose:
        print("\n".join((f"{num}) - '{dirs}' removed - already exists in cache"
                        for num, dirs in enumerate(intersect, 1))))
    elif args.quiet: 
        return None
    else: 
        if intersect: 
            print(f'{len(intersect)}\
director{"y" if len(intersect) == 1 else "ies"} already exists in cache')

def dirs_remove_message(dirs_to_remove):
    if args.verbose:
        print("\n".join((f"\ {num}) - '{dirs}' removed from cache" 

...
def src_add(cache_file, src, dirs_new=[]):
    ...
    try:
        ...
        for dir_path in dirs_new:
            ...
            if not SIM: cache_file[src].update({added_file_path: [file_hash, file_time]})     
        ...
        if directories:
            ...
            for dir_path in directories:
                ...
                if not SIM: cache_file[src].update({new_file_path: [file_hash, file_time]})
...
def src_update(cache_file, src):
    ...
    try:
        ...
        for dir_path in dirs_existing:
            ...
            if not SIM: cache_file[src].update({updated_file_path: [file_hash, file_time]})     
...
def src_swap(cache_file, src, swap_file):
    ...
    """
    if not lambdas.is_file_exist_and_accessible(swap_file): messages.src_not_existing_message_and_exit("s")
